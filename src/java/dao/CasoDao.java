...
@ManagedBean
@RequestScoped
public class CasoDao extends Dao {
    ...
    public void registrarCaso(Caso c) {
        ...
        try {
            ...
            String insert = "Insert into cas ( numCaso, canBloque, describloque, ubibloque, descrilamina, cantLami, ubilamina,  fechaRegis, estado,responsable, docPaciente, nombrePaciente, observacion) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    public List<Caso> listar() {
        ...
        try {
            this.conectar();
            ResultSet rs;
            String select = "select idcaso,docPaciente,nombrePaciente,numcaso,canBloque,describloque,ubibloque,descrilamina,cantLami,ubilamina,fechaRegis,estado,responsable,observacion  from cas ORDER BY idCaso DESC ";
...
//    
    ...
    public Caso getCasosID(Caso c) {
        ...
        try {
            this.conectar();
            String select = " SELECT  idcaso, docPaciente,nombrePaciente, numCaso, canBloque, describloque, cantLami, descrilamina, fechaRegis, responsable,  ubibloque, ubilamina, observacion  from cas where idcaso=?";
            PreparedStatement st = this.getCon().prepareStatement(select);
            st.setInt(1, c.getId());
            rs = st.executeQuery();            
...
//    }
    ...
    public void modificarpresta(Caso c) {
        ...
        try {
             ...
             */
            String update = "update cas set estado=?, RespPrestamo=?   WHERE idcaso=?";
    ...
    public void modificar(Caso c) {
        ...
        try {
            ...
            String update = "update cas set nombrePaciente=?, docPaciente=?, numCaso=?, canBloque=?, cantLami=?, fechaRegis=?,ubibloque=?, ubilamina=?, descrilamina=?, describloque=?  where idcaso=?";
    ...
    public void eliminar(Caso c) {
        try {
            this.conectar();
            String delete = "delete from cas where idcaso=?";
       ...
       public void consultarcaso(Caso c) {
        ...
        try {
            this.conectar();
            ResultSet rs;
            String select = "select * from cas where numcaso=? ";
            PreparedStatement st = this.getCon().prepareStatement(select);
    ...
    public List<Caso> listarOrden() {
        ...
        try {
            this.conectar();
            ResultSet rs;
            String select = "SELECT idcaso,docPaciente,nombrePaciente,numcaso,canBloque,describloque,ubibloque,descrilamina,cantLami,ubilamina,fechaRegis,estado,responsable,observacion FROM dbo.cas order by numcaso desc";
            PreparedStatement st = this.getCon().prepareStatement(select);
            rs = st.executeQuery();
            while (rs.next()) {
